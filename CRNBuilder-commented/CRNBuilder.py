...
class ChemicalReactorNetwork:
    ...
    def generate_inlets(self, Ny, Nz, y, z, T, my, mz, inlets, cluster_id, boundary_data):          #inlets is list of lists containing info to each inlet
        ...
        for n in range(len(inlets)):
            ...
            try:                                        #checks if the following code produces an error
                properties = ['U', 'T', 'rho', 'C']
                for property_ in properties:
                    boundary_data[inlet][property_]     #checks if entry to property exists in dictionary for the current inlet
            except KeyError:
                boundary_data_present = False
            if not boundary_data_present:               #not all properties found in boundary data dictionary
                for i in range(N):
                   ...
                   #ind is tuple describing location on of cell on outer borders
                    if not lower_bound <= x[ind] <= upper_bound: #if the current location is not within an area of an inlet then continue to next cell on outer border
                        continue
                    T_mean += T[ind]
                    if f * mx[ind] < 0:             #checking direction of massflow
                    ...
                    mass_flowrate[cluster_id[ind]] += f * max(mx[ind], 0.)    #why only positive massflow values added? this might end up with negative values in massflowrate
                ...
                i0 = 0
                for i in range(N):          #iterating through all cells on current boundary
                    ind = tuple([base_ind[k] * i if base_ind[k] > 0 else base_ind[k] for k in range(len(base_ind))])    #calculate position of in arary
                    if x[ind] > lower_bound:
                        i0 = i              #io is first cell within area of inlet (not on border of inlet)
                        break
                for i in range(len(boundary_data[inlet]['U'])): #iterating through number of cells within area of inlet
                    ind = tuple([base_ind[k] * i + i0 if base_ind[k] > 0 else base_ind[k] for k in range(len(base_ind))])   #ind now location in array of cells that are within inlet area, adding i0 because i=0 when first cell within inlet area is looked at
                    ...
                    if f * boundary_data[inlet][mx_name][i] < 0:
                        print(f"WARNING: f * boundary_data[inlet]['{mx_name}'][i] < 0, there must be a problem in the inlet reading. Check generate_inlet under ChemicalReactorNetwork in "
                              f"CRNBuilder.py")
                    if cluster_id[ind] not in mass_flowrate:            #check if cluster is already a key in mass_flowrate dictionary
                        mass_flowrate[cluster_id[ind]] = 0
                    mass_flowrate[cluster_id[ind]] += f * boundary_data[inlet][mx_name][i]              #add massflow into system at current cell to dictionary mass_flowrate
    ...
    def generate_outlets(self, Ny, Nz, y, z, outlets, my, mz):          #outlets: list of outputs with data to each output from parse_input()
        ...
        for n in range(len(outlets)):
            ...
            if outlets[n][1][0] == 0:           #checking which side outlet is located on
                for i in range(Ny):
                    if not lower_bound <= y[i, 0] <= upper_bound:       #checking whether the cell currently looked at is within the area of the outlet
                        continue
    ...
    def generate_reactors(self, Ny, Nz, n_clusters, cluster_id, pxr_score, T, axial_n, V, vy, vz, y, z):
        ...
        for n in range(n_clusters):
            ...
            for i in range(Ny):         #connecting reactor to CRN Outlet
                for j in range(Nz):
                    if self.outlet_id[i, j] == 0 or n != cluster_id[i, j]: #if current cell not connected to an output or current cell not within the reactor that we are currently looking at
                        continue
                    outlet_already_added = False
                    for reactor_outlet in self.reactors[-1].outlet: #looking at outlets of the last reactor that was created
                        if type(reactor_outlet).__name__ == 'CRNOutlet' and reactor_outlet.id_ == self.outlet_id[i, j] - 1:             #checking if the outlets is a CRNOutlet and if current cell is connected to that CRNoutlet
                            outlet_already_added = True
                            break
                    if not outlet_already_added: #if no outlet was added yet for this cell (not reactor)
                        ...
                        self.reactors[-1].outlet.append(self.crn_outlets[self.outlet_id[i, j] - 1]) #connect the outlet that is indicated in the outlet_id array
    ...
    def generate_splitters(self):
        ...
        for crn_object in self.reactors + self.crn_inlets:
            if len(crn_object.outlet) > 1:
                ...
                splitting_ratio = []
                for outlet in crn_object.outlet: # list of outlet connected to reactor, in this list CRN Outlets come before reactors
                    if isinstance(crn_object, Reactor):
                        if isinstance(outlet, Reactor): # case reactor to reactor
                            ...
                            #append massflow between reactor and outlet to this reactor (also reactor), done for each outlet for reactor
                        elif isinstance(outlet, CRNOutlet):
                            splitting_ratio.append(outlet.mass_flowrate) #why add entire mass_flowrate from outlet, the outlet could connected to more than one reactor hence not all of the massflow comes from this reactor
                        else:
    ...
    def generate_streams(self):
        ...
        #for crn_object in [o for o in self.crn_objects if not isinstance(o, CRNInlet)]: #iterating through all CRN objects except CRN inlets, first reactors, mixers, splitters, inlets , outlets
        for crn_object in list:  # iterating through all CRN objects except CRN inlets, first reactors, mixers, splitters, inlets , outlets

            for inlet in crn_object.inlet.copy(): #iterating through all inlets of current object
                if isinstance(inlet, CRNInlet):
                    continue
                elif not isinstance(inlet, Stream): #if inlet to object is a reactor, mixer or splitter
                    self.streams.append(Stream(counter, [inlet], [crn_object])) #create stream object, inlet of stream as original inlet and outlet the object currently looked at
                    #3rd paramater is outlet list, so current object is set as outlet of stream
                    crn_object.inlet.remove(inlet)
                    crn_object.inlet.append(self.streams[-1])
                    inlet.outlet.remove(crn_object)
                    inlet.outlet.append(self.streams[-1]) #stream is set as outlet for current object
...
def main():
    ...
    np.save('outlet_id.npy' , crn.outlet_id)
