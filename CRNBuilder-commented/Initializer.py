...
def initialize(case_name):
    ...
    if os.path.isdir(os.path.join(path, "case")):
        ...
        while prec > 3:
            ...
            for yf in cells_norm[:, radial_dir]:  #goes through all y coordinates of cells --> checks if they are
            ...
            print(f" Found {Ny = }, {Nz = }")
            if Ny == Ny0 and Nz == Nz0: # checks if the given number of cells in y/z direction matches with what is read
                break
            else:
        ...
        # Initialize matrices
        y = np.empty((Ny, Nz), dtype=float) #empty numpy array Ny rows, Nz columns
        ...
        for k in range(len(cells[:, 0])): #iterate through number of cells
            i = y_temp.index(round(cells_norm[k, radial_dir], prec))  #which clumn (radial) current cell is in?
            j = z_temp.index(round(cells_norm[k, axial_dir], prec))   #which row (axial) current
            y[i, j] = cells[k, radial_dir]      #created matrix with just y coordinates of cells
            z[i, j] = cells[k, axial_dir]       #created matric with just z coordinated of cells
            V[i, j] = V_of[k]
