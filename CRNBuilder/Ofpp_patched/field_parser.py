...
def parse_boundary_content(content):
    ...
    for boundary, (n1, n2) in bd.items():
        ...
        while True:
            ...
            elif b'zeroGradient' in lc:
                pd[b'value'] = 'zeroGradient'
            n += 1
            if n > n2:
                break
        data[boundary] = pd
...
def parse_data_nonuniform(content, n, n2, is_binary):
    ...
    :return: data
    """
    if ';' in content[n].decode('utf-8'):
        c = content[n].decode('utf-8').split(None, 3)
        num = int(c[3].split('(')[0])
        if not is_binary:
            if 'scalar' in c[2]:
                data = np.array(c[3].split('(')[1].rstrip(');\n\r').split(' '), dtype=float)
            else:
                data = np.array([ln.split() for ln in c[3].lstrip(f'{num}').lstrip('(').rstrip(');\n\r').split(') (')], dtype=float)
                # data = np.array([ln[1:-2].split() for ln in content[n + 3:n + 3 + num]], dtype=float)
        else:
            raise RuntimeError('Not supported, this must be implemented. Check field_parser.py, function parse_data_nonuniform()')
    else:
        num = int(content[n + 1])
        if not is_binary:
            if b'scalar' in content[n]:
                ...
                data = np.array([[a.strip(b'()') for a in ln.split()] for ln in content[n + 3:n + 3 + num]], dtype=float)
                # data = np.array([ln[1:-2].split() for ln in content[n + 3:n + 3 + num]], dtype=float)
        else:
            nn = 1
